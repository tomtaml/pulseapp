...
function validate(body){
  ...
  if(!Array.isArray(body.trust_values)||body.trust_values.length!==3||body.trust_values.some(v=>integerInRange(v,1,5)===null))return "Three valid trust responses are required.";
  for(const key of ["accessibility_understanding","wireless_acceptance","bidirectional_participation"]){if(integerInRange(body[key],1,5)===null)return `Invalid ${key}.`;}
  return null;
}
...
function researchPayload(body,env){
  ...
  const payload={
    ...
    workshop_code:clean.workshop_code,
    ...
    sus_values:clean.sus_values.map(Number),
    trust_values:clean.trust_values.map(Number),
  ...
  if(env.FREE_TEXT_ENABLED==="true")payload.optional_note=safeString(clean.optional_note,500);
  return payload;
}
...
async function handleSubmit(request,env){
  ...
  const trust=avg(clean.trust_values);
  ...
  await env.DB.prepare(`INSERT INTO submissions (id,app_version,variant,workshop_code,participant_group,language,comprehension_score,sus_completed,sus_score,trust_score,accessibility_understanding,wireless_acceptance,bidirectional_participation,payload_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(id,clean.app_version,clean.variant,clean.workshop_code,clean.participant_group,clean.language,comprehension,1,sus,trust,clean.accessibility_understanding,clean.wireless_acceptance,clean.bidirectional_participation,JSON.stringify(clean)).run();
