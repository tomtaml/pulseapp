...
if(v12Enabled){
  ...
  try{const saved=JSON.parse(sessionStorage.getItem(storageKey)||"null");if(saved?.session_ref&&saved?.update_token){creds=saved;scenario=saved.scenario||null;}}catch{}
  ...
  function publishSession(){window.PULSE_OPS_SESSION={...scenario,session_ref:creds?.session_ref||null,state:currentState};window.dispatchEvent(new CustomEvent("pulse:ops-session",{detail:window.PULSE_OPS_SESSION}));renderDev();}
  function renderDev(){
  ...
  async function register(){
    ...
    scenario=d.session;creds={session_ref:d.session.session_ref,update_token:d.update_token,scenario:d.session};sessionStorage.setItem(storageKey,JSON.stringify(creds));currentState=d.session.state||"DOCKING";publishSession();
  }
  ...
  function syncScreen(){
    ...
    if(/Demo valmis|Demo complete|Kiitos|Thank you/i.test(text)&&!ended){ended=true;const last=window.PULSE_CHARGING_LAST_SNAPSHOT||{};send({...last,state:"SESSION_ENDED",power_kw:0,direction:"idle"});}
  }
  window.addEventListener("pulse:charging-snapshot",e=>{const s=e.detail;if(!alignmentReady)return;if(["CHARGING","V2G_AVAILABLE","V2G_ACTIVE","RECHARGING","READY_TO_DEPART","OVERRIDDEN"].includes(s?.state))send(s);});
