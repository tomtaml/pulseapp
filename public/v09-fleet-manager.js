const q09=new URLSearchParams(location.search);
if((q09.get("variant")||"fi-fleet")==="fi-fleet"){
  const screen=document.querySelector("#screen");
  const workshop=(q09.get("workshop")||"DEMO").replace(/[^A-Za-z0-9_-]/g,"").slice(0,32)||"DEMO";
  ...
  function rolePage(){
    ...
    document.querySelectorAll('input[name="participant_group"]').forEach(input=>{
      if(input.dataset.v09Bound)return;
      input.dataset.v09Bound="1";
      input.addEventListener("change",()=>{sessionStorage.setItem(ROLE_KEY,input.value);rolePage();});
    });
    document.querySelector(".v09-role-preview")?.remove();
    if(checkedRole()!=="fleet_manager")return;
    const box=document.createElement("div");box.className="v09-role-preview";
  ...
  function serviceRequirements(){
    ...
    setLead("Tarkastele samaa Tampereen N1-jakeluskenaariota organisaation näkökulmasta. Tässä vaiheessa arvioidaan, mitä langattoman lataus- ja V2G-palvelun pitäisi vähintään taata kalustolle.","Review the same Tampere N1 delivery scenario from the organisation's perspective. Here the task is to judge what the wireless charging and V2G service should at minimum guarantee for the fleet.");
    ...
    if(!card){
      ...
      card.innerHTML=`<div class="v09-card-head"><strong>${tr("Tampere · N1-jakelukalusto","Tampere · N1 delivery fleet")}</strong><span>${tr("Työpajaskenaario","Workshop scenario")}</span></div>
      <div class="v09-guarantee-grid">
        <div><strong>🛡 ${tr("Liikkuminen","Mobility")}</strong><span>${tr("Seuraava toimitus ja suojattu lähtövaraus menevät verkkopalvelun edelle.","The next delivery and protected departure reserve take priority over grid service.")}</span></div>
        <div><strong>🔋 ${tr("Akun suojaus","Battery protection")}</strong><span>${tr("V2G toimii vain ennalta sovituissa akku- ja käyttörajoissa.","V2G operates only within agreed battery and operating limits.")}</span></div>
        <div><strong>❄ ${tr("Talvikäyttö","Winter operation")}</strong><span>${tr("Häiriötilanteelle on turvallinen keskeytys ja toimiva vararatkaisu.","Winter faults have a safe stop and workable fallback process.")}</span></div>
        <div><strong>📋 ${tr("Todennettavuus","Verifiability")}</strong><span>${tr("Lähtövalmius, energiasiirto, ohitukset ja häiriöt voidaan todentaa palveluraportista.","Departure readiness, energy transfer, overrides and faults can be verified from service records.")}</span></div>
      </div>`;
  ...
  function serviceLevel(){
    ...
    relabelByIndex("constraint_owner",[
      ["Kalusto-organisaatio määrittää sopimustason säännön","Fleet organisation defines the contractual rule"],
      ["Ajojärjestely säätää päivittäisen tarpeen sopimusrajoissa","Operations adjusts daily needs within contract limits"],
      ["Kuljettaja voi muuttaa rajaa sovituissa poikkeuksissa","Driver may adjust the reserve in agreed exceptions"],
      ["Jaettu sääntö: sopimusrajat + operoinnin tilannepäätös","Shared rule: contract limits + operational decision"]
    ]);
  ...
  function contractPage(){
    ...
    if(!card){
      ...
      card.innerHTML=`<div class="v09-card-head"><strong>${tr("Kaluston V2G-palvelusopimus","Fleet V2G service agreement")}</strong><span>${tr("Kuvitteellinen työpajamalli","Illustrative workshop model")}</span></div>
      <div class="v09-contract-grid">
        <div><span>${tr("Suojattu lähtövaraus","Protected departure reserve")}</span><strong>65%</strong></div>
        <div><span>${tr("Vaikutus toimitukseen","Delivery impact")}</span><strong>0 min</strong></div>
        <div><span>${tr("Aikaisempi käyttöönotto","Early recall")}</span><strong>${tr("sallittu","allowed")}</strong></div>
        <div><span>${tr("Akun suojaus","Battery protection")}</span><strong>${tr("sovituissa rajoissa","agreed limits")}</strong></div>
      </div>
      <div class="v09-contract-rule"><strong>${tr("Hallinta ja vastuu","Control and accountability")}</strong><span>${tr("Aktivointi, ohitus, häiriöt ja vastuut kirjataan niin, että palvelutaso voidaan todentaa.","Activations, overrides, faults and accountability are recorded so the service level can be verified.")}</span></div>
      <div class="v09-comp"><div><span>${tr("Esimerkkihyvitys","Illustrative compensation")}</span><strong>0,25 €/kWh</strong></div><small>${tr("Vain käyttöliittymätestin sopimusärsyke — ei markkinahinta eikä Tampereen lopullinen korvausmalli.","A contract stimulus for UI testing only — not a market price or final Tampere compensation model.")}</small></div>`;
      const firstField=field("v2g_authorisation");firstField?.insertAdjacentElement("beforebegin",card);
    }
    ...
    relabelByIndex("v2g_authorisation",[
      ["Kuljettaja vahvistaa jokaisen aktivoinnin","Driver confirms every activation"],
      ["Kalusto-organisaatio hyväksyy ennalta rajat, joiden sisällä V2G on sallittu","Fleet organisation pre-approves the limits within which V2G is allowed"],
      ["Ajojärjestely vahvistaa yksittäisen aktivoinnin","Operations confirms each activation"],
      ["Automaattinen sovituissa rajoissa, kuljettajalla/operoinnilla ohitus","Automatic within agreed limits, with driver/operations override"]
    ]);
  ...
  function evidencePage(){
    ...
    setLead("Seuraa yhtä nopeutettua lataus–V2G–latausjaksoa. Fleet manager -näkökulmasta kysymys on siitä, näkyykö raportista riittävästi palvelutason todentamiseen — yksi onnistunut jakso ei vielä osoita pitkäaikaista luotettavuutta.","Follow one accelerated charge–V2G–recharge session. From a fleet-manager perspective, the question is whether the record contains enough evidence to verify service performance — one successful session does not establish long-term reliability.");
    let note=document.querySelector(".v09-evidence-note");
    if(!note){note=document.createElement("div");note.className="v09-evidence-note";note.innerHTML=`<strong>${tr("SLA-tarkistuksessa pitäisi näkyä","An SLA check should show")}</strong><span>${tr("lähtövaraus ja lähtöaika · energia ajoneuvoon/verkkoon · V2G-aktivointi ja ohitukset · häiriö- ja palautumistiedot","departure reserve and readiness · energy to/from vehicle · V2G activation and overrides · fault and recovery records")}</span>`;document.querySelector(".v07-cycle-card")?.insertAdjacentElement("afterend",note);}
  ...
  function winterRisk(){
    ...
    const strong=fault?.querySelector("strong");if(strong)setText(strong,tr("Langaton lataus keskeytyi turvallisesti","Wireless charging stopped safely"));
    const p=fault?.querySelector("p");if(p)setText(p,tr("Lumi ja loska estävät latausalueen turvallisen tunnistuksen. Ajoneuvon varaus on 67 %, suojattu lähtövaraus 65 % ja seuraavaan toimitukseen on 25 minuuttia. Tässä skenaariossa 65 % riittää seuraavaan suunniteltuun toimitukseen.","Snow and slush prevent safe verification of the charging area. Battery is 67%, the protected reserve is 65%, and 25 minutes remain until the next delivery. In this scenario, 65% is sufficient for the assigned delivery."));
    ...
    relabelByIndex("fault_decision",[
      ["Salli yksi ohjattu uudelleenyritys aikarajan sisällä, sitten vararatkaisu","Allow one guided retry within the time budget, then use fallback"],
      ["Keskeytä langaton palvelu tältä pysähdykseltä ja turvaa seuraava toimitus","End wireless service for this stop and protect the next delivery"],
      ["Avaa tekninen incidentti palveluntarjoajalle ja kirjaa häiriö SLA-seurantaan","Open a provider technical incident and log the fault for SLA monitoring"],
      ["Siirrä lataus varikolle ja merkitse langaton sessio epäonnistuneeksi","Move charging to the depot and record the wireless session as unsuccessful"]
    ]);
    ...
    relabelByIndex("fault_owner",[
      ["Latauspalvelun / toimittajan tekninen vastuutaho","Charging-service / supplier technical owner"],
      ["Kalusto-organisaatio","Fleet organisation"],
      ["Jaettu vastuu ennalta sovitun rajapinnan mukaan","Shared responsibility under an agreed interface"],
      ["Palvelusopimuksessa nimetty SLA-vastuutaho","SLA owner named in the service agreement"]
    ]);
  }
  ...
  function comprehension(){
    ...
    relabelByIndex("c3",[
      ["Aina kun ajoneuvo on pysäköitynä","Whenever the vehicle is parked"],
      ["Vain kun suojattu lähtövaraus ja seuraavan toimituksen aikaraja voidaan säilyttää","Only when the protected reserve and next-delivery deadline can remain protected"],
      ["Vasta kun akku on ladattu täyteen","Only after the battery is fully charged"],
  ...
  function trust(){
    ...
    const labels={
      trust_reliability:["Voisin harkita palvelua hankintaan, jos se osoittaa sovitun luotettavuuden myös talviolosuhteissa.","I could consider procuring the service if it demonstrates the agreed reliability under winter conditions."],
      trust_predictability:["Sopimus ja raportointi tekisivät latauksen ja V2G:n vaikutukset ajoneuvojen käytettävyyteen riittävän ennakoitaviksi.","The agreement and reporting would make charging and V2G impacts on vehicle availability sufficiently predictable."],
      control_confidence:["Organisaatiolla olisi riittävä hallinta lähtövaraukseen, aktivointirajoihin ja V2G:n ohitukseen.","The organisation would have sufficient control over departure reserve, activation limits and V2G override."],
      failure_recovery_confidence:["Vastuut, vararatkaisu ja eskalointi olisivat riittävän selkeät teknisen häiriön aikana.","Accountability, fallback and escalation would be sufficiently clear during a technical fault."],
      wireless_use_intention:["Voisin harkita langattoman latauksen käyttöönottoa jakelukalustossa, jos palvelutaso osoitetaan käytännössä.","I could consider deploying wireless charging for the delivery fleet if the service level is demonstrated in practice."],
      v2g_acceptance_under_guarantees:["Voisin hyväksyä V2G:n kalustosopimukseen, jos liikkumistakuu, akun suojaus, korvaus ja vastuut määritellään selkeästi.","I could accept V2G in a fleet agreement if mobility protection, battery safeguards, compensation and accountability are clearly defined."]
    };
    Object.entries(labels).forEach(([name,[fi,en]])=>setText(legend(name),tr(fi,en)));
  }

  function apply(){
    if(is.welcome()){sessionStorage.removeItem(ROLE_KEY);return;}
    rolePage();
    if(!manager())return;
